...
TEST(AvmInputsTest, ValuesInColumns)
{
    ...
    // Global variables
    EXPECT_EQ(flat[col0_offset + AVM_PUBLIC_INPUTS_GLOBAL_VARIABLES_ROW_IDX], pi.globalVariables.blockNumber);

    // Start tree snapshots
    EXPECT_EQ(flat[col0_offset + AVM_PUBLIC_INPUTS_START_TREE_SNAPSHOTS_ROW_IDX],
              pi.startTreeSnapshots.l1ToL2MessageTree.root);
    EXPECT_EQ(flat[col1_offset + AVM_PUBLIC_INPUTS_START_TREE_SNAPSHOTS_ROW_IDX],
